...
using UnityEngine;
    ...
    public class DubinsMath  
    {
        ...
        public static float driveDistance = 0.005f;


        //The radius the car can turn 360 degrees with
        
        //public static float turningRadius = 2f;
        // public float turningRadius;


        //Calculate center positions of the Right circle
        public static Vector3 GetRightCircleCenterPos(Vector3 carPos, float heading, float _turningRadius)
        {
            Vector3 rightCirclePos = Vector3.zero;

            //The circle is 90 degrees (pi/2 radians) to the right of the car's heading
            rightCirclePos.x = carPos.x + _turningRadius * Mathf.Sin(heading + (Mathf.PI / 2f));
            rightCirclePos.z = carPos.z + _turningRadius * Mathf.Cos(heading + (Mathf.PI / 2f));
        ...
        public static Vector3 GetLeftCircleCenterPos(Vector3 carPos, float heading, float _turningRadius)
        {
            Vector3 rightCirclePos = Vector3.zero;
            ...
            out Vector3 goalTangent)
        {
            ...
            float xT1 = startCircle.x + _turningRadius * Mathf.Cos(theta);
            float zT1 = startCircle.z + _turningRadius * Mathf.Sin(theta);
            ...
            out Vector3 goalTangent)
        {
            ...
            float theta = Mathf.Acos((2f * _turningRadius) / D);
            ...
            float xT1_tmp = startCircle.x + 2f * _turningRadius * Mathf.Cos(theta);
            float zT1_tmp = startCircle.z + 2f * _turningRadius * Mathf.Sin(theta);
            ...
            out Vector3 middleCircle)
        {
            ...
            float theta = Mathf.Acos(D / (4f * _turningRadius));
            ...
            float x = startCircle.x + 2f * _turningRadius * Mathf.Cos(theta);
            float y = startCircle.y;
            float z = startCircle.z + 2f * _turningRadius * Mathf.Sin(theta);
            ...
            startTangent = middleCircle + V2 * _turningRadius;
            goalTangent = middleCircle + V3 * _turningRadius;
        }
            ...
            bool isLeftCircle)
        {
            ...
            float arcLength = Mathf.Abs(theta * _turningRadius);
            ...
            bool isTurningRight)
        {
            for (int i = 0; i < segments; i++)
            {
                ...
                if (isTurning)
                {
                    ...
                    //Update the heading
                    theta += (driveDistance / _turningRadius) * turnParameter;
                }
                pathTheta.Add(theta * Mathf.Rad2Deg);
